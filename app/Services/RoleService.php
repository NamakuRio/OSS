...
class RoleService
{
    public function store(Request $request)
    {
        ...
        try {
            $data = [
                'name' => $request->name,
                'login_destination' => formatLoginDestination($request->login_destination),
            ];
    ...
    public function setDefault(Request $request)
    {
        ...
        try {
            ...
            if($roles->count() == $success_data) {
                DB::commit();
                return ['status' => 'success', 'message' => 'Berhasil mengubah default peran.'];
            }

            DB::rollback();
            return ['status' => 'success', 'message' => 'Gagal mengubah default peran.'];
        } catch (Exception $e) {
            DB::rollback();
            return ['status' => 'error', 'message' => $e->getMessage()];
        }
        ...
        foreach ($rows as $key => $row) {
            ...
            foreach ($row as $action) {
                ...
                if ($action == 0) {
                    ...
                    $view .= '<input type="checkbox" class="custom-switch-input" name="permission[]" value="' . $action['id'] . '" id="customSwitch' . $action['id'] . '" ' . $checked . '>';
    ...
    {
        $data['login_destination'] = formatLoginDestination($data['login_destination']);
